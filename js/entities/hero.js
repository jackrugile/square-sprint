...
$.hero.prototype.render = function () {
  if (this.chargingTail) {
    $.ctx.beginPath();
    $.ctx.moveTo(
      this.chargeTailTween.before[0],
      this.chargeTailTween.before[1]
    );
    $.ctx.lineTo(this.x, this.y);
    $.ctx.lineWidth(1);
    $.ctx.strokeStyle(
      "hsla(" +
        $.game.state.level.hue +
        ", 100%, 80%, " +
        (1 - this.chargeTween.progress) +
        ")"
    );
    $.ctx.stroke();
    ...
    $.ctx.arc(
      this.x + $.rand(-1, 1),
      this.y + $.rand(-1, 1),
      Math.max(0, this.radius * 3 + $.rand(-5, 5)),
      this.chargeTweenAngle - Math.PI * 0.4 + $.rand(-0.1, 0.1),
      this.chargeTweenAngle + Math.PI * 0.4 + $.rand(-0.1, 0.1),
      false
    );
...
$.hero.prototype.charge = function (x, y) {
  ...
  this.chargeTailTween = $.game
    .tween(this.chargeTail)
    .to({ x: x, y: y }, 1.8, "outExpo");
  ...
  $.game.sound.setPlaybackRate(
    sound,
    0.25 + (this.chargeTweenDistance / $.game.width) * 0.75
  );
};

$.hero.prototype.warp = function (x, y) {
  if (
    this.checkWallCollision(x, y, false) ||
    !this.checkScreenCollision(x, y)
  ) {
    var sound = $.game.playSound("error1");
  ...
  $.game.sound.setPlaybackRate(
    sound,
    0.9 + (this.warpDistance / $.game.width) * 1.1
  );
};
