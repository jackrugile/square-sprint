...
$.enemy.prototype.checkCollision = function () {
  ...
  if (aabb) {
    ...
    $.game.state.hero.chargingTail &&
    $.segmentIntersect(tl, tr, h1, h2)
  ) {
    // check top left - top right
    return true;
  } else if (
    $.game.state.hero.chargingTail &&
    $.segmentIntersect(tr, br, h1, h2)
  ) {
    // check top right - bottom right
    return true;
  } else if (
    $.game.state.hero.chargingTail &&
    $.segmentIntersect(br, bl, h1, h2)
  ) {
    // check bottom right- bottom left
    return true;
  } else if (
    $.game.state.hero.chargingTail &&
    $.segmentIntersect(bl, tl, h1, h2)
  ) {
