...
$.statePlay.render = function (dt) {
  ...
  if (this.shake.translate || this.shake.rotate) {
    $.ctx.translate(
      $.game.width / 2 + $.rand(-this.shake.translate, this.shake.translate),
      $.game.height / 2 + $.rand(-this.shake.translate, this.shake.translate)
    );
  ...
  if (this.levelFlashTick > 0.005) {
    $.ctx.fillStyle(
      "hsla(0, 0%, 100%, " + this.levelFlashTick / this.levelFlashTickMax + ")"
    );
    ...
    $.ctx.fillStyle(
      "hsla(" +
        this.level.hue +
        ", 90%, " +
        (90 - (this.levelFlashTick / this.levelFlashTickMax) * 40) +
        "%, " +
        this.levelFlashTick / (this.levelFlashTickMax / 3) +
        ")"
    );
  ...
  if (this.enemyFlashTick) {
    $.ctx.fillStyle(
      "hsla(0, 0%, " + $.rand(0, 100) + "%, " + $.rand(0, 0.075) + ")"
    );
    $.ctx.fillRect(0, 0, $.game.width, $.game.height);
  }
  ...
  if (this.endStateTick > 0) {
    $.ctx.fillStyle(
      "hsla(0, 0%, 100%, " + this.endStateTick / this.endStateTickMax + ")"
    );
    $.ctx.fillRect(0, 0, $.game.width, $.game.height);
  }
  ...
  $.ctx.textAlign("right");
  $.ctx.fillText(
    "LEVEL " + this.levelNumber + "/" + this.levelTotal,
    $.game.width - 14,
    577
  );
