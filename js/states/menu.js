...
$.stateMenu.render = function (dt) {
  ...
  $.ctx.textAlign("left");
  $.ctx.fillText(
    "FASTEST RUN / " + $.msToString($.storage.get("fastestRun") * 1000),
    620,
    445
  );
  $.ctx.fillText(
    "TOTAL TIME / " + $.msToString($.storage.get("totalTime") * 1000),
    620,
    470
  );
  $.ctx.fillText(
    "TOTAL CLICKS / " + $.formatCommas($.storage.get("totalClicks")),
    620,
    495
  );
  ...
  $.ctx.drawImage(
    $.game.images["light"],
    $.game.width / 2 - 900 + Math.sin(this.tick / 150) * 300,
    $.game.height / 2 - 700 + Math.cos(this.tick / 175) * 75
  );
