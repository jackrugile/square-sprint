...
window.Whammy = (function () {
  function h(a, b) {
    for (
      ...
      var g = [],
        f = 0;
      do g.push(a[e]), (f += a[e].duration), e++;
      while (e < a.length && 3e4 > f);
      var h = 0,
        g = {
          id: 524531317,
          data: [
  ...
  function r(a) {
    for (
      var b = a[0].width, c = a[0].height, e = a[0].duration, d = 1;
      d < a.length;
      d++
    ) {
      if (a[d].width != b) throw "Frame " + (d + 1) + " has a different width";
      if (a[d].height != c)
        throw "Frame " + (d + 1) + " has a different height";
      if (0 > a[d].duration || 32767 < a[d].duration)
        throw (
          "Frame " +
          (d + 1) +
  ...
  function n(a) {
    ...
    for (var c = 0; c < a.length; c += 8) b.push(parseInt(a.substr(c, 8), 2));
  ...
  function m(a, b) {
    for (var c = [], e = 0; e < a.length; e++) {
      ...
      if ("string" == typeof d) {
        for (var g = new Uint8Array(d.length), f = 0; f < d.length; f++)
          g[f] = d.charCodeAt(f);
        d = g;
      }
  ...
  function p(a, b) {
    ...
    for (var c = 0; c < a.length; c++)
      "object" == typeof a[c] ? p(a[c], b) : b.push(a[c]);
    return b;
  }
  ...
  function t(a) {
    ...
    return (
      [a.trackNum | 128, a.timecode >> 8, a.timecode & 255, b]
        .map(function (a) {
  ...
  function q(a) {
    for (
      var b = a.RIFF[0].WEBP[0], c = b.indexOf("\u009d\u0001*"), e = 0, d = [];
      4 > e;
      e++
    )
      d[e] = b.charCodeAt(c + 3 + e);
    e = (d[1] << 8) | d[0];
    c = e & 16383;
    e = (d[3] << 8) | d[2];
    return {
  ...
  l.prototype.add = function (a, b) {
    if ("undefined" != typeof b && this.duration)
    ...
    if (!/^data:image\/webp;base64,/gi.test(a))
...
PLAYGROUND.Application.prototype = {
  ...
  emitLocalEvent: function (event, data) {
    ...
    if ((!this.firstBatch || this.loader.ready) && this[event])
      this[event](data);
  },
  ...
  enableMousewheel: function () {
    ...
    for (var i = eventNames.length; i; ) {
      self.element.addEventListener(
        eventNames[--i],
        PLAYGROUND.Utils.throttle(function (event) {
...
PLAYGROUND.SoundWebAudioAPI.prototype = {
  ...
  play: function (name, loop) {
    ...
    this.setPosition(
      bufferSource,
      this.masterPosition.x,
      this.masterPosition.y,
      this.masterPosition.z
    );
...
PLAYGROUND.Touch.prototype = {
  ...
  touchmove: function (e) {
    for (var i = 0; i < e.changedTouches.length; i++) {
      ...
      this.x = this.touchmoveEvent.x =
        ((touch.pageX - this.elementOffset.x - this.app.offsetX) /
          this.app.scale) |
        0;
      this.y = this.touchmoveEvent.y =
  ...
  touchstart: function (e) {
    for (var i = 0; i < e.changedTouches.length; i++) {
      ...
      this.x = this.touchstartEvent.x =
        ((touch.pageX - this.elementOffset.x - this.app.offsetX) /
          this.app.scale) |
        0;
      this.y = this.touchstartEvent.y =
        ((touch.pageY - this.elementOffset.y - this.app.offsetY) /
          this.app.scale) |
        0;
  ...
  touchend: function (e) {
    for (var i = 0; i < e.changedTouches.length; i++) {
      ...
      this.touchendEvent.x =
        ((touch.pageX - this.elementOffset.x - this.app.offsetX) /
          this.app.scale) |
        0;
      this.touchendEvent.y =
        ((touch.pageY - this.elementOffset.y - this.app.offsetY) /
          this.app.scale) |
        0;
...
PLAYGROUND.VideoRecorder.prototype = {
  setup: function (args) {
    ...
    this.layer = cq(
      (this.region[2] * this.scale) | 0,
      (this.region[3] * this.scale) | 0
    );
  },
  ...
  step: function (delta) {
    if (this.encoder) {
      ...
      this.app.screen
        ...
        .lineWidth(8)
        .strokeStyle("#c00")
        .strokeRect(0, 0, this.app.screen.width, this.app.screen.height)
        .restore();
    }
  },
...
(function () {
  ...
  cq.Color.prototype = {
    ...
    mix: function (color, amount) {
      ...
      for (var i = 0; i < 4; i++) this[i] = cq.mix(this[i], color[i], amount);
